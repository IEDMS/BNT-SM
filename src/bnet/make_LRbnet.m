...
function [bnet] = make_LRbnet(structure)
...
for i=1:length(s.eclasses),
        ...
        %xxxxxxxxxxxxxxxxx
        if strcmpi(eclass.type, 'root'),
            bnet.CPD{e} = root_CPD(bnet,bnet.rep_of_eclass(e));   
        %xxxxxxxxxxxxxxxx
        elseif strcmpi(eclass.type, 'discrete'),
        ...
        elseif strcmpi(eclass.type, 'softmax'),
            bnet.CPD{e} = softmax_CPD(bnet,bnet.rep_of_eclass(e));
        end
end
